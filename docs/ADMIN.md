...
- Не ставьте `localhost` и не копируйте хост `postgres` из docker-compose — на Bothost это имя не резолвится.
- Включите addon Postgres и Redis в панели и скопируйте **их** `POSTGRES_HOST` / `REDIS_HOST` (или готовый URL).
